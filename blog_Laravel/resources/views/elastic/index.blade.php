        ...
        <div class="col-md-12 col-md-offset-0">
            <div class="panel panel-default">
                ...
                <div class="panel-heading text-warning">
                    ...
                    <p><a href="{{ route('elastic-indexing') }}"><button class="btn btn-success"> Make Elastic indexing table(N/A)</button></a></p> 
                    <p></p>                  
                </div>
                ...
                <div class="panel-body">
					...
					<div class="col-sm-6 col-xs-12 {{ (isset($_GET['elastic-search'])) ? 'bg-danger':''}}"></br>    
					    <h4>Elastic Search on table{shop_simple} <i class="fa fa-search-plus" style="font-size:26px"></i></h4> 
						<form class="form-horizontal" method="get" action="{{url('elastic')}}">
					...
					<div class="col-sm-12 col-xs-12">
                        ...
                        @else
							<p>Your Elastic data is here! You searched for <b><i> {{ $_GET['elastic-search']}} </i></b>. Found {{count($elasticResults->results)}} result. </p>
							<br>
							...
							@foreach($elasticResults->results as $res) <!-- $results ia an array -->
                                ...
                                <div class="col-sm-12 col-xs-12 list-group-item alert alert-success">
							        ...
							        <p class="list-group-item"> SQL ID(for link):   <b> {{ $res->shop_id->raw }}    </b></p> <!--SQL table column id to make a href -->                  <!-- For single post(without loop) was => $elasticResults->results[0]->shop_id->raw} -->	
							        <p class="list-group-item"> <a href="{{route('elas-one-product', ['id' => $res->shop_id->raw])}}">View One</a> </p> <!-- Link to view this one result -->
								    <p class="list-group-item"> Product:            <b> {{ $res->shop_title->raw }} </b></p> <!--SQL table column shop_title  -->                        <!-- For single post(without loop) was => $elasticResults->results[0]->shop_title->raw} -->
