    ...
    public function up()
    {
		...
		if (!Schema::hasTable('shop_transactions')) { //my fix for migration
          Schema::create('shop_transactions', function (Blueprint $table) {
			...
			$table->decimal('gross', 6, 2); // DECIMAL equivalent with a precision and scale //this means that your column is set up to store 6 places (scale), with 2 to the right of the decimal (precision). A sample would be 1234.56.
			...
			$table->unsignedInteger('orderId')->nullable();  //product Id from table {shop_simple}
