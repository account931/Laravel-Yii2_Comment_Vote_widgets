    ...
    public function up()
    {
		...
		if (!Schema::hasTable('shop_order_item')) { //my fix for migration
          Schema::create('shop_order_item', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('order_id')->nullable(); //from table {shop_orders_main}
			
			//Create Foreign key for table {shop_simple}	
			$table->unsignedInteger('product_id')->nullable();  //product Id from table {shop_simple}
			...
			$table->string('uuid', 222)->nullable();  //Эквивалент VARCHAR с длинной 222 // 
